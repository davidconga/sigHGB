...
const TIPOS = {
  ...
  junta_medica: 'Junta',
  fisioterapeutico: 'Fisioterapia',
  informacao_clinica: 'Info Clínica',
  nota_alta: 'Nota Alta',
  guia_transferencia: 'Transferência',
}
...
export default function RelatoriosScreen({ navigation }) {
  ...
  async function load() {
    try {
      const { data } = await api.get('/relatorios', { params: { search, per_page: 30 } })
      setItems(data.data || [])
    ...
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      {loading ? <ActivityIndicator color={colors.hgb[600]} style={{ marginTop: 20 }} /> : (
        <FlatList
          ...
          ListEmptyComponent={<Text style={{ textAlign: 'center', color: colors.slate[400], marginTop: 40 }}>Sem relatórios.</Text>}
          renderItem={({ item }) => (
            <TouchableOpacity onPress={() => navigation.navigate('RelatorioShow', { id: item.id })}>
              <Card>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                  <Text style={{ fontWeight: '700', color: colors.hgb[700] }}>{item.numero}</Text>
                  <StatusBadge status={item.status} />
                </View>
                <Text style={{ fontSize: font.sm, marginTop: 4 }}>{item.paciente?.nome}</Text>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 4 }}>
                  <Text style={{ fontSize: font.xs, color: colors.slate[500] }}>{TIPOS[item.tipo] || item.tipo}</Text>
                  <Text style={{ fontSize: font.xs, color: colors.slate[500] }}>{item.data_emissao?.slice(0, 10)}</Text>
                </View>
              </Card>
            </TouchableOpacity>
          )}
